...
class OrderBook {
	...
	/**
	 * @param orderId
	 *            uniquely identifies/timestamps an order
	 * @param sideStr
	 *            buy/sell order
	 * @param orderTypeStr
	 *            limit/market/cancel/update order
	 * @param volume
	 *            specifies the quantity
	 * @param price
	 *            price of the stock
	 */
	public void processOrder(int orderId, String sideStr, String orderTypeStr,
	...
	private void matchOrder(Order order) {
		...
		if (order.getSide() == Side.BUY) {
			...
			if (order.getOrderType() == OrderType.LIMIT
					&& order.getVolume() > 0) {
				// If the order is limit and there is an non-zero volume then
				// add to order table
	 ...
	 * 
	 * @param orderId
	 *            id of an existing order
	 * @param orderType
	 *            type of the existing order
	 * @param volume
	 *            quantity of the new order
	 * @param price
	 *            price of the new order
	 */
	private void modifyOrder(int orderId, OrderType orderType, int volume,
			double price) {
		if (orderMap.containsKey(orderId)) {
			...
			if (price == order.getPrice()) {
				...
				// If the price is modified then add the order to a different
				// list
	...
	private List<Trade> matchSellOrder(Order order) {
		...
		while (order.getVolume() > 0 && !buys.isEmpty()) {
			...
			if (order.getOrderType() == OrderType.LIMIT
				...
				// If the order is limit order and max price is less than the
				// limit price
				break;
			}
			...
			while (order.getVolume() > 0 && curOrder != null) {
				...
				if (curOrder.getVolume() <= order.getVolume()) {
					...
					trade.Init(order.getOrderId(), curOrder.getOrderId(),
							curOrder.getVolume(), curOrder.getPrice());
					this.removeOrder(curOrder);
					curOrder = ol.getHead();
				} else {
					trade.Init(order.getOrderId(), curOrder.getOrderId(),
							order.getVolume(), curOrder.getPrice());
	...
	private List<Trade> matchBuyOrder(Order order) {
		...
		while (order.getVolume() > 0 && !sells.isEmpty()) {
			...
			if (order.getOrderType() == OrderType.LIMIT
				...
				// If the order is limit order and min price is greater than the
				// limit price
				break;
			}
